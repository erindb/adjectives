...
p = ggplot(subset(webppl_output, aes(x=value, y=probability, linetype=group,
                             colour=prior_distribution)) +
  geom_line(lwd=0.7) +
  facet_grid(cost_of_adjective ~ speaker_lambda, scale="free") +
  theme_bw(10) +
...
# p = ggplot(actual_priors, aes(x=value, y=probability, colour=prior_distribution)) +
#   geom_line() +
#   facet_wrap(~ prior_distribution)
# print(p)
```
...
# p = ggplot(data=subset(model,
#                        utterance == "adjective" & cost_of_adjective == 1 & speaker_lambda == 1),
#            aes(x=value, y=probability, colour=variable)) +
#   geom_line() +
#   facet_wrap(~ prior_distribution) +
#   ggtitle("Posterior distributions for model with lambda 1 cost 1") +
#   theme_bw(10) +
#   theme(panel.grid=element_blank())
# print(p)
```
...
```{r, echo=F}
erin = expected_thetas$expected
anthea = c(8.87, 6.39, 3.31, 6.96, 5.61, 7.43, 4.42, 3.44, 3.53, 4.06, 8.57, 5.55, 4.82, 6.22, 6.20)

compare = data.frame(
  erin = erin,
  anthea = anthea,
  item = expected_thetas$prior_distribution
  )
compare$item = revalue(compare$item, c("bas"="Basketball", "book"="Book pages", "bur"="Burger", "cig"="Cigarettes", "eat"="Time spent eating", "edu" = "Time in education","girl"="Girlfriends", "hair" = "Visits to hairdresser's", "kid" = "Children", "mov" = "Movies in cinema", "move" = "Age of moving out", "poem"="Poem lines", "shoe"="Pairs of shoes", "trav"= "Commuting time", "tv"= "Hours watching TV"))
p = ggplot(compare, aes(x=erin, y=anthea, colour=item)) +
  geom_point(size=3) +
  geom_text(aes(label=item), hjust=-0.1) +
  xlab("Erin thresholds (intervals)") +
  ylab("Anthea thresholds (intervals)") +
  xlim(4, 6.3)
print(p)
cor.test(erin, anthea)

anthea_percentiles = subset(read.csv("thetaintervals.csv", header=T), quantifier == "many")
compare_percentile = data.frame(
  erin = expected_thetas$percentile,
  anthea_low = anthea_percentiles$HDIlow,
  anthea_high = anthea_percentiles$HDIhigh,
  item = factor(as.character(expected_thetas$prior_distribution))
)
ggplot(compare_percentile, aes(x=erin, y=anthea_low, colour=item)) +
  geom_errorbar(aes(x=erin, ymin=anthea_low, ymax=anthea_high)) +
  geom_text(aes(label=item), hjust=-0.1)

# compare_percentile2 = data.frame(
#   theta = c(compare_percentile$erin, compare_percentile$anthea_low, compare_percentile$anthea_high),
#   version = c(rep("erin", 15), rep("anthea", 30)),
#   item = c(rep(compare_percentile$item, 3))
#   )
# p = ggplot(compare_percentile2, aes(x=item, y=theta, colour=item)) +
#   geom_point()
# print(p)

# bad_ones = c("move", "cig", "bas", "edu")
bad_ones = c()
p = ggplot(subset(compare_percentile, !(item %in% bad_ones)),
           aes(x=item, ymin=anthea_low, ymax=anthea_high, y=2, colour=item)) +
  geom_linerange(size=1.5) +
  geom_point(aes(x=item, y=erin, size=7)) +
  ylim(0, 1)
print(p)

# anthea_percentiles = subset(read.csv("thetaintervals.csv", header=T), quantifier == "many")
# compare_percentile = data.frame(
#   erin = c(expected_thetas$percentile,expected_thetas$percentile),
#   anthea = c(anthea_percentiles$HDIlow, anthea_percentiles$HDIhigh),
#   lowhigh = c(rep("low", 15), rep("high", 15)),
#   item = c(as.character(expected_thetas$prior_distribution),
#            as.character(expected_thetas$prior_distribution))
# )
# p = ggplot(compare_percentile, aes(x=erin, y=anthea, colour=factor(item), group=lowhigh)) +
#   geom_errorbar(x=erin, y) +
#   geom_text(aes(label=item), hjust=-0.1) +
#   xlab("Erin thresholds (percentiles)") +
#   ylab("Anthea thresholds (percentiles)")
# print(p)
```